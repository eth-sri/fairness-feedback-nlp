    ...
    for target_label in [0,1,2,4,5,8,9,10,11,12,13,15,16,17,18]:
        for remove_label in [0,1,2,4,5,8,9,10,11,12,13,15,16,17,18]:
            ...
            generate(model_type="Bart",removal_mode="drop_thresh",q="mean",remove_label=remove_label,target_label=target_label,n_beams=5,max_length=64,
                     batch_size=4,total_items=1000000,max_generations=args.max_generations,add_successes_only=True,overwrite_labels=True,
                     n_batches=1000000,generator_path=args.source_folder+"/Bart_test_label_attention_mean_640",
                     classifier_path=args.source_folder+"/Roberta_Kaggle_64",add_label=True,gpt_mode=None,word_replacement_mode="50",
                     eval_classifier_path=args.source_folder+"/Roberta_Kaggle_64",attention_layer=10,save=args.source_folder+"/style_transfer_pairs/"+str(remove_label)+"_"+str(target_label))